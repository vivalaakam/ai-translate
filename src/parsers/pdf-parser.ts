...
export class PdfParser {
  ...
  private getPdfInfo(): { title: string; author: string; pageCount: number } {
    let output: string;
    try {
      output = execFileSync('pdfinfo', [this.filePath], {
        encoding: 'utf-8',
   ...
   * pdftoppm with -f/-l generates files like: <prefix>-NN.png (2-digit zero-padded)
   * or <prefix>-NNNN.png depending on total pages.
   */
